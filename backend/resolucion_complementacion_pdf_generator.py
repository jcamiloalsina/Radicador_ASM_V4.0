...
def generar_resolucion_complementacion_pdf(data: dict) -> bytes:
    ...
    def dibujar_tabla_predio_complementacion(predio_data, es_cancelacion=False, vigencia=None,
                                              ...
                                              propietarios_override=None, matricula_override=None,
                                              direccion_override=None, destino_override=None):
        """
        ...
        props = propietarios_override if propietarios_override is not None else predio_data.get('propietarios', [])
        ...
        if props and len(props) > 0:
            for prop in props:
                ...
                tipo_doc_prop = prop.get('tipo_documento', prop.get('tipo_documento', 'C'))
                c.setFont(font_normal, fuente_tabla - 1)
                c.drawCentredString(x + cancel_cols[2]/2, y_position - cancel_row_h/2 - 2, tipo_doc_prop)
                x += cancel_cols[2]

                # NRO. DOC.
                c.rect(x, y_position - cancel_row_h, cancel_cols[3], cancel_row_h, fill=0, stroke=1)
                nro_doc = prop.get('numero_documento', prop.get('documento', ''))
            ...
            c.rect(x, y_position - cancel_row_h, cancel_cols[1], cancel_row_h, fill=0, stroke=1)
            draw_cell_text(c, solicitante_nombre[:40], x, cancel_cols[1], y_position, cancel_row_h, default_fs=7)
            x += cancel_cols[1]

            c.rect(x, y_position - cancel_row_h, cancel_cols[2], cancel_row_h, fill=0, stroke=1)
            c.setFont(font_normal, fuente_tabla - 1)
            c.drawCentredString(x + cancel_cols[2]/2, y_position - cancel_row_h/2 - 2, "C")
            x += cancel_cols[2]

            c.rect(x, y_position - cancel_row_h, cancel_cols[3], cancel_row_h, fill=0, stroke=1)
            c.setFont(font_normal, fuente_tabla - 1)
            c.drawCentredString(x + cancel_cols[3]/2, y_position - cancel_row_h/2 - 2, str(solicitante_documento)[:15])
            x += cancel_cols[3]

            c.rect(x, y_position - cancel_row_h, cancel_cols[4], cancel_row_h, fill=0, stroke=1)
            c.setFont(font_normal, fuente_tabla - 1)
            c.drawCentredString(x + cancel_cols[4]/2, y_position - cancel_row_h/2 - 2, "")
            y_position -= cancel_row_h
        ...
        p_direccion = direccion_override if direccion_override else predio_data.get('direccion', direccion)
        p_destino = destino_override if destino_override else predio_data.get('destino_economico', destino_economico)
        ...
        draw_cell_text(c, str(p_codigo_hom)[:12] if p_codigo_hom else "", x, predio_cols[0], y_position, predio_row_h)
        x += predio_cols[0]
        ...
        c.drawCentredString(x + predio_cols[2]/2, y_position - predio_row_h/2 - 2, str(p_destino)[:1] if p_destino else "A")
        x += predio_cols[2]
        ...
        p_matricula = matricula_override if matricula_override else predio_data.get('matricula_inmobiliaria', matricula)
