...
def generate_resolucion_m3_pdf(
    ...
    try:
        ...
        for firma_path in ["/app/logos/firma_dalgie_blanco.png", "/app/backend/logos/firma_dalgie_blanco.png", "logos/firma_dalgie_blanco.png"]:
            if os.path.exists(firma_path):
                ...
                firma_loaded = True
                break
        if not firma_loaded and imagen_firma_b64:
            firma_dalgie = ImageReader(io.BytesIO(base64.b64decode(imagen_firma_b64)))
        if not firma_loaded and not imagen_firma_b64:
    ...
    def dibujar_tabla_predio(titulo_seccion, destino_val, avaluo_val, es_cancelacion=True):
        ...
        # Primera fila de encabezados - row height 20
        row_h = 20
        ...
        draw_cell_text(c, codigo_predial[:30], x, col_widths[0], y_position, row_h, default_fs=7)
        x += col_widths[0]
        ...
        c.setFont(font_normal, 7)
        c.drawCentredString(x + col_widths[4]/2, y_position - row_h/2 - 2, destino_val)
        y_position -= row_h

        # Segunda fila of encabezados - predio data - row height 20
        predio_row_h = 20
        c.setFillColor(colors.HexColor('#e8e8e8'))
        ...
        draw_cell_text(c, codigo_hom[:15] if codigo_hom else "", x, col_widths2[0], y_position, predio_row_h)
        x += col_widths2[0]
        ...
        draw_cell_text(c, avaluo_fmt, x, col_widths2[5], y_position, predio_row_h)
        x += col_widths2[5]

        # VIG. FISCAL
        c.rect(x, y_position - predio_row_h, col_widths2[6], predio_row_h, fill=0, stroke=1)
        c.setFont(font_normal, 5)
        c.drawCentredString(x + col_widths2[6]/2, y_position - predio_row_h/2 - 2, vigencia)
        y_position -= predio_row_h
