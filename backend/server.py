...
def get_email_template(titulo: str, contenido: str, radicado: str = None, tipo_notificacion: str = "info", boton_texto: str = None, boton_url: str = None) -> str:
    ...
    """
    frontend_url = os.environ.get('FRONTEND_URL', 'https://property-sync-10.preview.emergentagent.com')
...
@api_router.post("/actualizacion/proyectos/{proyecto_id}/vincular-datos-existentes")
async def vincular_datos_existentes_proyecto(
    proyecto_id: str,
    current_user: dict = Depends(get_current_user)
):
    """Vincula los datos existentes de Conservación (GDB y R1/R2) al proyecto de Actualización"""
    if current_user['role'] not in [UserRole.ADMINISTRADOR, UserRole.COORDINADOR]:
    ...
    # Contar geometrías GDB existentes
    total_gdb = await db.gdb_geometrias.count_documents({"municipio": municipio})
    
    # Contar predios R1/R2 existentes
    total_predios = await db.predios.count_documents({"municipio": municipio})
    
    # Contar construcciones
    total_construcciones = await db.gdb_construcciones.count_documents({"municipio": municipio})
    
    # Actualizar proyecto con datos vinculados
    await db.proyectos_actualizacion.update_one(
        {"id": proyecto_id},
        {"$set": {
            "datos_vinculados": True,
            "base_grafica_total_predios": total_gdb,
            "info_alfanumerica_total_registros": total_predios,
            "total_construcciones": total_construcciones,
            "datos_vinculados_en": datetime.now(timezone.utc),
            "updated_at": datetime.now(timezone.utc)
        }}
    )
    
    return {
        "message": f"Datos de {municipio} vinculados exitosamente",
        "estadisticas": {
            "geometrias_gdb": total_gdb,
            "predios_r1r2": total_predios,
            "construcciones": total_construcciones
        }
    }


@api_router.get("/actualizacion/proyectos/{proyecto_id}/datos-municipio")
async def get_datos_municipio_proyecto(
    proyecto_id: str,
    current_user: dict = Depends(get_current_user)
):
    """Obtiene los datos del municipio asociado al proyecto (para el visor)"""
    if current_user['role'] not in [UserRole.ADMINISTRADOR, UserRole.COORDINADOR, UserRole.GESTOR]:
        raise HTTPException(status_code=403, detail="No tiene permiso para esta operación")
    ...
    # Obtener límites del municipio para centrar el mapa
    limite = await db.limites_municipales.find_one({"municipio": municipio}, {"_id": 0})
