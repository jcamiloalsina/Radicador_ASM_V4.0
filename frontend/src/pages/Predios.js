...
export default function Predios() {
  ...
  // Función para registrar eliminación manual
  const handleRegistrarEliminacion = async () => {
    if (savingRegistro) return;
    
    // Validaciones
    if (!registroEliminacion.codigo_predial_nacional.trim()) {
      toast.error('Debe ingresar el código predial nacional');
      return;
    }
    if (!registroEliminacion.municipio.trim()) {
      toast.error('Debe seleccionar el municipio');
      return;
    }
    if (!registroEliminacion.resolucion.trim()) {
      toast.error('Debe ingresar el número de resolución');
      return;
    }
    if (!registroEliminacion.motivo.trim()) {
      toast.error('Debe ingresar el motivo de eliminación');
      return;
    }
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.post(`${API}/predios/eliminados/registrar`, {
        codigo_predial_nacional: registroEliminacion.codigo_predial_nacional.trim(),
        codigo_homologado: registroEliminacion.codigo_homologado.trim(),
        municipio: registroEliminacion.municipio.trim(),
        direccion: registroEliminacion.direccion.trim(),
        nombre_propietario: registroEliminacion.nombre_propietario.trim(),
        area_terreno: parseFloat(registroEliminacion.area_terreno) || 0,
        area_construida: parseFloat(registroEliminacion.area_construida) || 0,
        avaluo: parseFloat(registroEliminacion.avaluo) || 0,
        destino_economico: registroEliminacion.destino_economico.trim(),
        radicado_eliminacion: registroEliminacion.radicado_eliminacion.trim(),
        resolucion: registroEliminacion.resolucion.trim(),
        fecha_resolucion: registroEliminacion.fecha_resolucion,
        motivo: registroEliminacion.motivo.trim()
      }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      
      toast.success('Eliminación registrada exitosamente');
      ...
      setShowRegistrarEliminacion(false);
      
      // Recargar lista de eliminados
      fetchPrediosEliminados();
    } catch (error) {
      toast.error(error.response?.data?.detail || 'Error al registrar eliminación');
    } finally {
